...
// Handler para crear la información de un postulante :
const createPostulanteHandler = async (req, res) => {
    ...
    const {
        ...
        observaciones, id_distrito, distrito,
    ...
    if (!id_distrito || isNaN(id_distrito)) errores.push('El ID de distrito es requerido y debe ser un entero')
    ...
    try {
        ...
        if (id_distrito === 0) {
            ...
            const distritosName = distritos.map(d => d.nombre);
            if (!distritosName.includes(distrito)) await createDistrito(distrito);
        }
        ...
        if (!response) {
            await deleteCv(req.file.filename);
            return res.status(400).json({
