...
const configurarCronJobs = async () => {
    ...
    cron.schedule('06 * * * *', async () => {
        ...
        // Cron Job en curso :
        console.log(`Ejecutando algoritmo de asistencia para el día ${diaString} a las ${hora}:06:00`);
        try {
            const create = await createAsistencias(diaString, hora);
            if (!create) console.warn(`No se toma asistencia a las ${hora}:06:00. Esperar a la siguiente hora...`);
            else console.log(`Algoritmo de asistencia ejecutado con éxito a las ${hora}:06:00`);

        } catch (error) {
            console.error(`Error al ejecutar crear asistencias para las ${hora}:06:00`, error);
        }
    });
};
